...
class StreamingDecisionTree:
    def __init__(self, min_samples_split=2, max_depth=100, grace_period=200, n_features=None, decay_rate=0.99, delta=0.05):
        ...
        self.decay_rate = 0.99  # Decay rate for statistics
        self.delta = delta  # Confidence parameter for Hoeffding Bound
    ...
    def _attempt_split(self, node, X, y, depth):
        ...
        for feat_idx in feat_idxs:
            ...
            for thr in thresholds:
                gain = self._information_gain(list(node.stats.keys()), X, feat_idx, thr)
                if gain > best_gain:
...
# Initialize the StreamingDecisionTree
tree = StreamingDecisionTree(min_samples_split=2, max_depth=10, grace_period=50, n_features=2, decay_rate=0.99, delta=0.05)
...
y_stream = (X_stream[:, 0] + X_stream[:, 1] > 1).astype(int)  # Label: 1 if sum of features > 1, else 0

# Stream the data one by one to the tree
print("Streaming data and training...")
for i in range(stream_size):
    tree.update(X_stream[i], y_stream[i])

    # Optionally, predict periodically
    if i % 100 == 0 and i > 0:
        sample = X_stream[i]
        prediction = tree.predict(sample)
        print(f"Sample {i}: True Label={y_stream[i]}, Predicted={prediction}")
...
for i, sample in enumerate(new_samples):
    prediction = tree.predict(sample)
    print(f"Sample {i}: Features={sample}, Predicted Label={prediction}")
